...
class AE33:
    ...
    def __init__(self, name: str, config: dict) -> None:
        ...
        self.name = name
        self.table_ext_dict = {'Data': 'dat', 'Log': 'log'}
        try:
            ...
            # Configure tcp/ip
            self.sockaddr = (config[name]['socket']['host'], config[name]['socket']['port'])
            ...
            self._data = ""
            self._data_begin_read_id = 0
            self._data_file_to_stage = ""
            self._log = ""
            self._log_begin_read_id = 0
            self._log_file_to_stage = ""
    ...
    def setup_schedules(self) -> None:
        """Set up the schedules for data acquisition, saving, and staging."""
        try:
    ...
    def _fetch_from_table(self, name: str, rows: Optional[int] = None, first: Optional[int] = None, last: Optional[int] = None) -> str:
        """Fetch data from a table, applying filters for rows and ranges."""
        try:
            if not name:
            ...
            return self._tcpip_comm(cmd=cmd, tidy=False)
    ...
    def set_datetime(self) -> None:
        ...
        try:
            ...
            self._tcpip_comm(cmd)
            self.logger.info(f"[{self.name}] DateTime of instrument set to: {cmd}")
    ...
    def _accumulate_new_data(self, table: str, sep="|") -> None:
        """Retrieve all records from the table that have not been read."""
        try:
            if table not in self.table_ext_dict:
            ...
            # Determine start point for reading
            begin_read_id = self._data_begin_read_id if table == 'Data' else self._log_begin_read_id
            if begin_read_id == 0:
                begin_read_id = int(self._tcpip_comm(cmd=f"MINID {table}", tidy=True))

            chunk_size = 1000
            while begin_read_id < maxid:
                end_read_id = min(begin_read_id + chunk_size, maxid)
                cmd = f"FETCH {table} {begin_read_id} {end_read_id}"
                ...
                begin_read_id = end_read_id

            # Set the read position for the table
            if table == 'Data':
    ...
    def _save_data(self, table: str) -> None:
        ...
        try:
            ...
            if content:
                ...
                if table == 'Data':
                    self._data_file_to_stage = str(file)
                elif table == 'Log':
                    self._log_file_to_stage = str(file)

        except Exception as err:
            self.logger.error(err)

    def _stage_file(self, table: str) -> None:
    ...
    def _save_and_stage_data(self) -> None:
        """Save and stage data for both 'Data' and 'Log' tables."""
        try:
    ...
    def print_ae33(self) -> None:
        ...
        try:
            ...
            data = self._tcpip_comm(cmd, tidy=True).split(sep="|")
    ...
    def tape_advances_remaining(self) -> str:
        """Retrieve tape advances remaining."""
        try:
            cmd = "$AE33:A"
            res = self._tcpip_comm(cmd, tidy=True).replace("\n", "")
            return res
