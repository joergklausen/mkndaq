...
#         # Some firmware variants also prepend a count word.
#         values_by_param: dict[int, bytes] = {}

#         def _word_u32(w: bytes) -> int:
#             return int.from_bytes(w, byteorder="big", signed=False)

#         words_use = list(words)

#         # Optional leading count word (common in other commands; observed on some builds for cmd=4 too)
#         if len(words_use) in {n_req + 1, 2 * n_req + 1}:
#             maybe_count = _word_u32(words_use[0])
#             if maybe_count in {n_req, 2 * n_req}:
#                 words_use = words_use[1:]
#                 n_words = len(words_use)

#         # values-only: [v1, v2, ...]
#         if n_words == n_req:
#             values_by_param = dict(zip(parameters, words_use))

#         # id/value pairs: [id1, v1, id2, v2, ...] or [id1..idN, v1..vN]
#         elif n_words == 2 * n_req:
#             ids_step = [_word_u32(words_use[i]) for i in range(0, n_words, 2)]
#             vals_step = [words_use[i] for i in range(1, n_words, 2)]
#             if ids_step == parameters:
#                 values_by_param = dict(zip(parameters, vals_step))
#             else:
#                 ids_head = [_word_u32(w) for w in words_use[:n_req]]
#                 vals_tail = words_use[n_req:]
#                 if ids_head == parameters and len(vals_tail) == n_req:
#                     values_by_param = dict(zip(parameters, vals_tail))
#                 else:
#                     raise ValueError(
#                         "Get Values response payload shape looks like id/value pairs, but returned IDs do not match request. "
#                         f"requested={parameters}, got_ids_head={ids_head[:min(10,len(ids_head))]}"
#                     )

#         # Fallback: if payload has extra leading words, try taking the last n_req as values-only
#         elif n_words > n_req and n_words < 2 * n_req:
#             tail = words_use[-n_req:]
#             self.logger.warning(
#                 f"[{self.name}] Get Values payload has {n_words} words for {n_req} requested. "
#                 "Using last n_req words as values-only."
#             )
#             values_by_param = dict(zip(parameters, tail))
...
#     def _tcpip_recv_acoem_packet(self, s: socket.socket, *, verbosity: int = 0) -> bytes:
#         """Receive a single ACOEM packet from a connected socket.

#         Some ACOEM/TCP deployments sit behind a Telnet-like server and may prepend negotiation bytes (0xFF...).
#         This receiver:
#           - discards any leading noise before the first STX (0x02)
#           - uses the 2-byte message length field (bytes 5..6) to determine the full packet size
#           - falls back to EOT (0x04) framing if the length field looks implausible or incomplete

#         Returns:
#             Raw packet bytes, including STX..EOT.

#         Raises:
#             socket.timeout if a complete packet is not received within the socket timeout.
#         """
#         buf = bytearray()
#         start = time.perf_counter()
#         deadline = start + float(self.socktout)

#         def _drop_to_stx(b: bytearray) -> int:
#             stx = b.find(0x02)
#             if stx < 0:
#                 if len(b) > 4096:
#                     del b[:-4]
...
#         def _try_len(b: bytearray) -> bytes | None:
#             if _drop_to_stx(b) < 0:
#                 return None
#             if len(b) < 6:
#                 return None
#             # Header sanity: STX, SID, CMD, ETX
#             if b[0] != 0x02:
#                 del b[0]
#                 return None
#             if b[3] != 0x03:
...
#             msg_len = int.from_bytes(bytes(b[4:6]), byteorder="big", signed=False)
#             total = 4 + 2 + msg_len + 1 + 1  # header + len + data + checksum + EOT

#             # Guard against nonsense lengths so we don't wait forever on garbage.
#             if msg_len > 65535 or total > 70000:
#                 del b[0]
#                 return None
...
#         def _try_eot(b: bytearray) -> bytes | None:
#             if _drop_to_stx(b) < 0:
#                 return None
#             eot = b.find(0x04, 0)
#             if eot < 0:
#                 return None
#             return bytes(b[: eot + 1])

#         while True:
#             pkt = _try_len(buf) or _try_eot(buf)
#             if pkt is not None:
#                 if verbosity > 2:
#                     self.logger.debug(f"ACOEM recv: {pkt!r}")
#                 return pkt
...
#             if not chunk:
#                 pkt = _try_len(buf) or _try_eot(buf)
#                 return pkt if pkt is not None else b""
#             buf.extend(chunk)

#     def _tcpip_comm(self, message: bytes, expect_response: bool=True, verbosity: int=0) -> bytes:
#         """
#         Send and receive data using the configured TCP/IP protocol.

#         For protocol == 'acoem', messages are binary ACOEM packets and responses are received as ACOEM packets.
#         For protocol == 'aurora', messages are ASCII commands and responses are CRLF-terminated.

#         Args:
#             message (bytes): bytes to send
#             expect_response (bool): if False, do not attempt to read a response
#             verbosity (int): 0..3

#         Returns:
#             bytes: response bytes (possibly empty)
#         """
#         self._tcpip_comm_wait_for_line()
#         try:
#             self._tcpip_line_is_busy = True
...
#                 if verbosity > 2:
#                     self.logger.debug(f"message sent: {message!r}")
#                 s.sendall(message)
...
#                     # Some servers send Telnet negotiation bytes (0xFF...) before STX.
#                     stx = rcvd.find(b"\x02")
#                     if stx > 0:
#                         rcvd = rcvd[stx:]
